...
export default function CreationsGallery() {
  const [selectedImage, setSelectedImage] = useState(null);
  const [currentIndex, setCurrentIndex] = useState(0);
  const { creations = [] } = useApi(); // <-- API data
  ...
  const goToPrev = () => {
    const newIdx = currentIndex === 0 ? creations.length - 1 : currentIndex - 1;
    setCurrentIndex(newIdx);
    setSelectedImage(creations[newIdx]);
  };

  const goToNext = () => {
    const newIdx = currentIndex === creations.length - 1 ? 0 : currentIndex + 1;
    setCurrentIndex(newIdx);
    setSelectedImage(creations[newIdx]);
  };
  ...
  return (
    <>
      {/* ===== GALLERY SECTION ===== */}
      <section
        className="py-16 md:py-20 bg-gradient-to-b from-[#0f172a] via-[#1e1b4b] to-[#161E31] text-white overflow-hidden"
        id="creations"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Header */}
          <div className="text-center mb-12 md:mb-16">
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-3">
              Our Latest <span className="text-[#F8B179]">Creations</span>
            </h2>
          ...
          <div className="mt-8 md:hidden text-center">
            <button
              onClick={() => openLightbox(0)}
              className="inline-flex items-center gap-2 px-6 py-3 bg-[#161E31] text-[#F8B179] font-medium rounded-full hover:bg-[#161E31] transition-shadow shadow-lg"
            >
              View Gallery
              <ChevronRight className="w-5 h-5" />
            </button>
              ...
              {creations.map((thumb, i) => (
                <button
                  ...
                  className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-all ${
                    i === currentIndex
                      ? "border-red-600 scale-110"
                      : "border-white/30 hover:border:border-white/60"
                  }`}
                >
                  <img
                    src={`${baseUrl}/${thumb.image}`}
